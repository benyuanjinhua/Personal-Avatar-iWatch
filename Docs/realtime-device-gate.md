...
## 卡点定位（白梦林 2026-08-23 最新决策）

- **真机证据不作为 ESS-1071 的验收卡点**。本清单是交给「白梦林」真机验证的
  执行/回贴清单，真机结果在发布阶段（ESS-1072）回贴父单 ESS-1057；它不阻断
  ESS-1071 自身的收口，也不反推回滚已合入的 PR。
- 合入 main 的卡点只有 `Docs/realtime-ci-gate.md` 的机器门禁（PR 合入卡点）。
