...
## 卡点定位（白梦林 2026-08-23 明确）

- **真机门禁是 ESS-1071 验收 #4 的硬卡点**：真机证据未回贴、未判通过前，
  ESS-1071 不得置 `done`，父单 ESS-1057 不得收口。
- 与 PR 合入的关系沿用仓库 R-02.5 约定：**真机验收是 issue 收口卡点，不是
  PR 合入卡点**；PR 合入 main 由 `Docs/realtime-ci-gate.md` 的机器门禁把关，
  真机不过只挡 issue 收口与发布，不反推回滚已合入的 PR。
