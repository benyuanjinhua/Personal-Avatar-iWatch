...
| `ERR_UPLINK_COMMIT_TIMEOUT` | — / 1008 | ESS-959: no `audio.commit` within `uplink_commit_timeout_ms` of `session_ready` (retriable). |
...
- `uplink_commit_timeout` (ESS-959: pre-commit watchdog fired)
...
| `uplink_commit_timeout_ms` | `90000` | ESS-959: how long a session may sit between `session_ready` and `audio.commit` before failing closed with `ERR_UPLINK_COMMIT_TIMEOUT` (retriable). Must stay above the client's VAD hard cap (`LocalVADEndpointer.maximumTurnMs`, 55 s) so a healthy full-length recording is never killed. `0` disables the watchdog. |
